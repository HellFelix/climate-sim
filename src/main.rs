...
fn setup(
    ...
    // Allocate zeroed data so Bevy can create the texture
    image.resize(size);
    let mini_handle = images.add(image);
    commands.insert_resource(MiniCamTexture(mini_handle.clone()));
        ...
        .with_children(|parent| {
            parent.spawn((
                ImageBundle {
                    ...
                    image: UiImage::new(mini_handle),
                    ..default()
                },
                Name::new("Subwindow Image"),
            ));
        });
}
