...
with $0<\epsilon<1$. Another important limitation of this simple radiation model is that it does not account for interactions with the atmosphere, such as scattering, diffusion, and reflection.
 ...
 \section{Results}
   After assembling the mathematical models from the theory into a single model, we simulated the system in several different configurations to see how various parts of the model affect the results.
 \subsection{Non rotating planet}
 Firsty we wanted to take a very simple case, a non rotating planet that rotates around its star with planet-axis perpendicular to the plane wich it rotates in.
 \begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/temperature_basic.png}
