...
\begin{document}
  ...
  \begin{abstract}
   ...
   Our goal with the model was to observe effects such as day–night cycles, seasonal variations, colder poles, and a warmer equator. Although the model failed to reproduce all of these features, it is thermally stable, and it clearly shows notable differences between day and night, summer and winter, as well as between equatorial and polar regions. It is also able to see biger trends such as the general effect of an increase of greenhouse gases.
  ...
  Since the Monte Carlo method is far too computationally heavy to calculate flux (Power per square length unit) within a reasonable time on a planet-sized system, simplifications must be made. We begin by separating the incoming solar radiation from the outgoing thermal radiation emitted by the planet. In this chapter, we discuss how we model the light approaching the Earth and what happens to it as it passes through the atmosphere.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[h!]{0.48\textwidth}
        ...
        \caption{Simulating temperatures on the surface of a non-rotating planet with an axis perpendicular to its orbital plane around its host star, with no heat diffusion.}
    ...
    \begin{subfigure}[h!]{0.48\textwidth}
        ...
        \caption{Simulating temperatures on the surface of a rotating planet with an axis perpendicular to its orbital plane around its host star, with no heat diffusion.}
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{Simulating temperatures on the surface of a rotating planet in an elliptical orbit around its host star, with no heat diffusion.}
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{Simulating temperatures on the surface of a rotating planet with a tilted axis and an elliptical orbit, with no heat diffusion.}
...
We then proceed to Figure~\ref{fig:planet_elliptic}, where the day–night cycle remains largely the same, but the maximum temperature now varies periodically. This occurs because the distance between the planet and its host star changes throughout the elliptical orbit (see Section~\ref{sec:slora_system}).
...
\begin{figure}[h!]
  ...
  \caption{Simulating temperatures on the surface of a rotating planet with a tilted axis and an elliptical orbit, with heat diffusion enabled.}
...
\begin{figure}[h!]
  ...
  \caption{Simulating temperatures on the surface of a rotating planet with a tilted axis and an elliptical orbit, with heat diffusion and an increased amount of greenhouse gases.}
...
Our simulation is far from accurate, but it manages to reproduce several real-world phenomena, which was our primary goal. It does so while using mathematical models that have been applied in real-world climate and weather simulations by NASA.
